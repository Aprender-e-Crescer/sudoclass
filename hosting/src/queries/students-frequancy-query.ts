...
export async function getAttendanceStatus() {
  ...
  try {
    const attendancesCollectionRef = collection(
      firestore,
      '/schoolMatrices/aQjvxCKlEuHc9YQEedCQ/subjects/zGTOAwnKJBjFSmayHxJo/attendances',
    )
    ...
    const allStudentsAttendance = querySnapshot.docs.map((docSnap) => {
      ...
      return {
        createdAt: createdAt
          ...
          : 'Data não disponível',
        students: data.students.map((studentEntry: { student: string; present: string }) => ({
