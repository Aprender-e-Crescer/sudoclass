...
export const Route = createFileRoute(
  '/_authenticated/charts/$idClass/materia-presences',
)({
...
function ChartsScreen() {
  ...
  const {
    data: presenceMateria,
    isLoading: isLoadingpresenceMateria,
    error: errorpresenceMateria,
  } = usePresenceMateriaQuery(idClass)

  if (isLoading || isLoadingpresenceMateria) {
    return (
      <div>
        <h1>Carregando...</h1>
      </div>
  ...
  if (error || errorpresenceMateria)
    return (
      <h1>
        Erro ao carregar os dados:{' '}
        {error?.message ?? errorpresenceMateria?.message}
      </h1>
    )
